...
function spatialFeatures = computeEnhancedSpatialFeatures(binaryModel)
    ...
    % 8. 定向变差函数（提高空间分布复杂度）
    maxLag = max(1, floor(min(size(binaryModel)) / 4));
    if maxLag >= 1
        variogramLags = unique(max(1, round(linspace(1, maxLag, min(6, maxLag)))));
    else
        variogramLags = 1;
    end
    spatialFeatures.directionalVariogram = computeDirectionalVariogram(binaryModel, variogramLags);

    % 9. 径向分布函数，捕捉多尺度空间结构
    spatialFeatures.radialDistribution = computeRadialDistribution(binaryModel, 12);

    % 10. 局部孔隙率熵，衡量局部空间复杂度
    spatialFeatures.localPorosityEntropy = computeLocalPorosityEntropy(binaryModel, [5, 9, 13]);

    % 11. Minkowski泛函，度量拓扑和几何属性
    spatialFeatures.minkowskiMeasures = computeMinkowskiFunctionals(binaryModel);

    % 12. 多尺度线要素概率，评估长程连通特征
    spatialFeatures.multiScaleLinealPath = computeMultiScaleLinealPath(binaryModel, [3, 5, 7, 11]);
end
...
function variogram = computeDirectionalVariogram(binaryModel, lags)
    % 计算定向半变异函数
    data = double(binaryModel);
    nLags = numel(lags);
    gammaX = zeros(nLags, 1);
    gammaY = zeros(nLags, 1);
    gammaZ = zeros(nLags, 1);

    for i = 1:nLags
        lag = lags(i);

        if lag < size(data, 1)
            diffX = data(1:end-lag, :, :) - data(1+lag:end, :, :);
            gammaX(i) = 0.5 * mean(diffX(:).^2);
        else
            gammaX(i) = NaN;
        end

        if lag < size(data, 2)
            diffY = data(:, 1:end-lag, :) - data(:, 1+lag:end, :);
            gammaY(i) = 0.5 * mean(diffY(:).^2);
        else
            gammaY(i) = NaN;
        end

        if lag < size(data, 3)
            diffZ = data(:, :, 1:end-lag) - data(:, :, 1+lag:end);
            gammaZ(i) = 0.5 * mean(diffZ(:).^2);
        else
            gammaZ(i) = NaN;
        end
    end
...
function values = interpolateVariogram(variogram, lags)
    % 将定向半变异函数插值到公共滞后距离
    directions = {'x', 'y', 'z'};
    interpolated = nan(numel(directions), numel(lags));

    for d = 1:numel(directions)
        dirVals = variogram.(directions{d});
        valid = ~isnan(dirVals);
        if sum(valid) >= 2
            interpolated(d, :) = interp1(variogram.lags(valid), dirVals(valid), ...
                lags, 'linear', 'extrap');
        elseif sum(valid) == 1
            interpolated(d, :) = dirVals(valid);
        end
    end

    values = mean(interpolated, 1, 'omitnan');
end

function radial = computeRadialDistribution(binaryModel, nBins)
    % 计算径向分布函数
    [x, y, z] = ind2sub(size(binaryModel), find(binaryModel));
    coords = [x, y, z];
    nPoints = size(coords, 1);

    if nPoints < 2
        radial = struct('binCenters', zeros(1, nBins), 'probability', zeros(1, nBins));
        return;
    end

    maxRadius = norm(size(binaryModel));
    edges = linspace(0, maxRadius, nBins + 1);
    binCenters = (edges(1:end-1) + edges(2:end)) / 2;

    % 随机采样对以控制复杂度
    maxPairs = 8000;
    nPairs = min(maxPairs, floor(nPoints * (nPoints - 1) / 2));
    distances = zeros(nPairs, 1);

    for i = 1:nPairs
        idx = randperm(nPoints, 2);
        diff = coords(idx(1), :) - coords(idx(2), :);
        distances(i) = norm(double(diff));
    end

    counts = histcounts(distances, edges, 'Normalization', 'probability');
    radial = struct('binCenters', binCenters, 'probability', counts);
end

function entropyInfo = computeLocalPorosityEntropy(binaryModel, windowSizes)
    % 计算不同尺度下的局部孔隙率熵
    entropyValues = zeros(1, numel(windowSizes));
    porosityDistributions = cell(1, numel(windowSizes));

    for i = 1:numel(windowSizes)
        w = windowSizes(i);
        kernel = ones(w, w, w);
        localSum = convn(double(binaryModel), kernel, 'valid');
        localPorosity = localSum / (w^3);

        bins = linspace(0, 1, 21);
        histVals = histcounts(localPorosity(:), bins, 'Normalization', 'probability');
        histVals(histVals == 0) = [];
        entropyValues(i) = -sum(histVals .* log(histVals + eps));
        porosityDistributions{i} = histVals;
    end

    entropyInfo = struct();
    entropyInfo.windowSizes = windowSizes;
    entropyInfo.entropy = entropyValues;
    entropyInfo.distributions = porosityDistributions;
end

function minkowski = computeMinkowskiFunctionals(binaryModel)
    % 计算Minkowski泛函
    minkowski = struct();
    minkowski.volumeFraction = mean(binaryModel(:));

    boundary = bwperim(binaryModel, 26);
    minkowski.surfaceArea = sum(boundary(:));

    se = ones(3, 3, 3);
    dilated = imdilate(binaryModel, se);
    eroded = imerode(binaryModel, se);
    minkowski.meanBreadth = (sum(dilated(:)) - sum(eroded(:))) / (6 * numel(binaryModel));

    try
        minkowski.eulerCharacteristic = bweuler(binaryModel, 26);
    catch
        minkowski.eulerCharacteristic = 0;
    end
end

function lineal = computeMultiScaleLinealPath(binaryModel, lengths)
    % 计算多尺度线要素概率
    lineal = struct();
    lineal.lengths = lengths;
    px = zeros(size(lengths));
    py = zeros(size(lengths));
    pz = zeros(size(lengths));
    ...
    for i = 1:numel(lengths)
        ...
        if L < size(data, 1)
            convX = convn(data, ones(L, 1, 1), 'valid');
            px(i) = mean(convX(:) == L);
        else
            px(i) = NaN;
        end
        ...
        if L < size(data, 2)
            convY = convn(data, ones(1, L, 1), 'valid');
            py(i) = mean(convY(:) == L);
        else
            py(i) = NaN;
        end
        ...
        if L < size(data, 3)
            convZ = convn(data, ones(1, 1, L), 'valid');
            pz(i) = mean(convZ(:) == L);
        else
            pz(i) = NaN;
        end
...
function displaySpatialFeatures(spatialFeatures)
    ...
    if isfield(spatialFeatures, 'directionalVariogram')
        fprintf('  定向变差函数均值: %.4f\n', ...
            mean(interpolateVariogram(spatialFeatures.directionalVariogram, ...
            spatialFeatures.directionalVariogram.lags), 'omitnan'));
    end
    if isfield(spatialFeatures, 'radialDistribution')
        fprintf('  径向分布熵: %.4f\n', ...
            -sum(spatialFeatures.radialDistribution.probability .* ...
            log(spatialFeatures.radialDistribution.probability + eps)));
    end
    if isfield(spatialFeatures, 'localPorosityEntropy')
        fprintf('  局部孔隙率熵: %.4f\n', ...
            mean(spatialFeatures.localPorosityEntropy.entropy));
    end
    if isfield(spatialFeatures, 'minkowskiMeasures')
        fprintf('  Minkowski泛函：体积分数 %.4f，表面积 %.2f，平均宽度 %.4f\n', ...
            spatialFeatures.minkowskiMeasures.volumeFraction, ...
            spatialFeatures.minkowskiMeasures.surfaceArea, ...
            spatialFeatures.minkowskiMeasures.meanBreadth);
    end
    if isfield(spatialFeatures, 'multiScaleLinealPath')
        fprintf('  多尺度线要素平均概率: %.4f\n', ...
            mean([spatialFeatures.multiScaleLinealPath.x, ...
            spatialFeatures.multiScaleLinealPath.y, ...
            spatialFeatures.multiScaleLinealPath.z], 'all', 'omitnan'));
    end
end
...
function mcmcModel = generateComprehensiveInitialModel(dims, targetPorosity, ...
    ...
    for c = 1:numCandidates
        % 1. 基于空间相关性生成基础场，并加入随机扰动以探索解空间
        corrScale = estimateCorrelationLength(targetTPC) * (0.85 + 0.3 * rand());
        anisotropySample = max(0, targetAnisotropy + 0.1 * randn());
        initialField = generateCorrelatedGaussianField(dims, corrScale, anisotropySample);
        ...
        if ~isempty(morphologyFeatures.sphericity)
            ...
            if targetSphericity > 0.6
                ...
                % 低球形度：应用各向异性滤波
                initialField = anisotropicFilter3D(initialField, anisotropySample);
            end
        end

        % 3. 阈值化以获得目标孔隙率
        threshold = quantile(initialField(:), 1 - targetPorosity);
        candidateModel = initialField > threshold;

        % 4. 形态与空间预处理
        candidateModel = applyTargetMorphology(candidateModel, morphologyFeatures);
        candidateModel = adjustClusterSizeDistribution(candidateModel, optParams);
        if isfield(spatialFeatures, 'minkowskiMeasures')
            candidateModel = enforceMinkowskiAlignment(candidateModel, spatialFeatures.minkowskiMeasures);
        end
        if isfield(spatialFeatures, 'radialDistribution')
            candidateModel = enforceRadialDistribution(candidateModel, spatialFeatures.radialDistribution);
        end

        % 5. 多阶段特征优化
        nOptSteps = 45;
        for step = 1:nOptSteps
            currentSpatial = computeEnhancedSpatialFeatures(candidateModel);
            currentMorph = computeDetailedMorphologyFeatures(candidateModel);
            ...
            if spatialMatch > 0.93 && morphMatch > 0.93
                break;
            end

            [dominantGap, gapType] = detectDominantSpatialGap(currentSpatial, spatialFeatures);
            if dominantGap > 0.12
                switch gapType
                    case 'minkowski'
                        if isfield(spatialFeatures, 'minkowskiMeasures')
                    ...
                    case 'variogram'
                        if isfield(spatialFeatures, 'directionalVariogram')
                            candidateModel = balanceDirectionalVariogram(candidateModel, spatialFeatures.directionalVariogram);
                        end
                    case 'entropy'
                        if isfield(spatialFeatures, 'localPorosityEntropy')
                            candidateModel = adjustLocalPorosityEntropy(candidateModel, spatialFeatures.localPorosityEntropy);
                        end
                    case 'lineal'
                        if isfield(spatialFeatures, 'multiScaleLinealPath')
                            candidateModel = balanceLinealPath(candidateModel, spatialFeatures.multiScaleLinealPath);
                        end
                end
            end

            if spatialMatch < morphMatch
                candidateModel = applySpatialCorrection(candidateModel, currentSpatial, spatialFeatures);
            else
                candidateModel = applyMorphologyCorrection(candidateModel, currentMorph, morphologyFeatures);
            end

            candidateModel = adjustPorosity(candidateModel, targetPorosity);
        end

        % 6. 精细化处理
        candidateModel = targetedInitialRefinement(candidateModel, optParams);

        % 7. 评估候选模型
        candidateScore = evaluateInitialCandidate(candidateModel, optParams);
    ...
    % 8. 最终严格匹配
    if isfield(spatialFeatures, 'minkowskiMeasures')
        model = enforceMinkowskiAlignment(model, spatialFeatures.minkowskiMeasures);
    end
    if isfield(spatialFeatures, 'radialDistribution')
        model = enforceRadialDistribution(model, spatialFeatures.radialDistribution);
    end
    if isfield(spatialFeatures, 'directionalVariogram')
        model = balanceDirectionalVariogram(model, spatialFeatures.directionalVariogram);
    end
    if isfield(spatialFeatures, 'localPorosityEntropy')
        model = adjustLocalPorosityEntropy(model, spatialFeatures.localPorosityEntropy);
    end
    model = targetedInitialRefinement(model, optParams);

    fprintf('  初始模型生成完成\n');
    mcmcModel = model;
end

function model = enforceMinkowskiAlignment(model, targetMinkowski)
    % 通过形态学运算逼近目标Minkowski泛函
    if isempty(targetMinkowski)
        return;
    end

    current = computeMinkowskiFunctionals(model);

    % 调整体积分数
    desiredVolume = targetMinkowski.volumeFraction;
    if desiredVolume > 0
        model = adjustPorosity(model, desiredVolume);
    end

    % 根据表面积差异选择平滑或强化操作
    surfaceDiff = current.surfaceArea - targetMinkowski.surfaceArea;
    se = strel('sphere', 1);
    if surfaceDiff > 0
        % 表面过于粗糙，执行平滑
        model = imclose(imopen(model, se), se);
    elseif surfaceDiff < 0
        % 表面过于光滑，增加细节
        model = imopen(imclose(model, se), se);
    end

    % Euler特征调整（通过添加/去除小团簇）
    if isfield(targetMinkowski, 'eulerCharacteristic')
        try
            currentEuler = bweuler(model, 26);
            if currentEuler < targetMinkowski.eulerCharacteristic
                % 增加孔隙连接
                dilated = imdilate(model, se);
                model = dilated;
            elseif currentEuler > targetMinkowski.eulerCharacteristic
                % 减少孔隙连接
...
function model = enforceRadialDistribution(model, targetRadial)
    % 通过局部膨胀/腐蚀匹配径向分布
    if isempty(targetRadial) || ~isfield(targetRadial, 'probability')
        return;
    end

    currentRadial = computeRadialDistribution(model, length(targetRadial.probability));
    minLen = min(length(currentRadial.probability), length(targetRadial.probability));
    if minLen == 0
        return;
    end

    lowScaleDiff = mean(currentRadial.probability(1:ceil(minLen/3)) - ...
        targetRadial.probability(1:ceil(minLen/3)));
    highScaleDiff = mean(currentRadial.probability(floor(2*minLen/3):minLen) - ...
        targetRadial.probability(floor(2*minLen/3):minLen));

    se = strel('sphere', 1);
    if lowScaleDiff > 0.01
        % 近程对过多，适度腐蚀
        model = imerode(model, se);
    elseif lowScaleDiff < -0.01
        % 近程对不足，适度膨胀
        model = imdilate(model, se);
    end

    if highScaleDiff < -0.01
        % 长程联系不足，增加桥接结构
        bridgeMask = rand(size(model)) < 0.01;
        model = model | bridgeMask;
    elseif highScaleDiff > 0.01
        % 长程联系过多，随机切断部分连通
        breakMask = rand(size(model)) < 0.01;
        model(breakMask) = false;
    end
end

function model = adjustLocalPorosityEntropy(model, targetEntropy)
    % 调整局部孔隙率熵，使其更接近目标
    if isempty(targetEntropy)
        return;
    end

    currentEntropy = computeLocalPorosityEntropy(model, targetEntropy.windowSizes);
    meanCurr = mean(currentEntropy.entropy);
    meanTarget = mean(targetEntropy.entropy);

    if meanCurr > meanTarget + 0.02
        % 熵过高，说明局部过于杂乱，执行强力平滑
        model = medfilt3(double(model), [3, 3, 3]) > 0.5;
    elseif meanCurr < meanTarget - 0.02
        % 熵过低，增加局部扰动
        perturbMask = rand(size(model)) < 0.015;
        model(perturbMask) = ~model(perturbMask);
    end
end

function model = balanceDirectionalVariogram(model, targetVariogram)
    % 调整模型的定向变差函数
    if isempty(targetVariogram) || ~isfield(targetVariogram, 'lags')
        return;
    end

    currentVar = computeDirectionalVariogram(model, targetVariogram.lags);
    errX = mean(currentVar.x - targetVariogram.x, 'omitnan');
    errY = mean(currentVar.y - targetVariogram.y, 'omitnan');
    errZ = mean(currentVar.z - targetVariogram.z, 'omitnan');

    sigma = max(0, [errX, errY, errZ]);
    sigmaVec = max([0.5, 0.5, 0.5], 0.5 * abs([errX, errY, errZ]) + 1);

    if any([errX, errY, errZ] > 0.02)
        % 误差为正，说明方差偏大，进行各向异性平滑
        smoothed = imgaussfilt3(double(model), sigmaVec);
        model = smoothed > 0.5;
    elseif any([errX, errY, errZ] < -0.02)
        % 误差为负，说明方差偏小，增强细节
        blurred = imgaussfilt3(double(model), 1.2);
        sharpened = double(model) + (double(model) - blurred);
        sharpened = min(max(sharpened, 0), 1);
        model = sharpened > 0.5;
    end
end

function model = balanceLinealPath(model, targetLineal)
    % 调整多尺度线要素概率
    if isempty(targetLineal) || ~isfield(targetLineal, 'lengths')
        return;
    end

    currentLineal = computeMultiScaleLinealPath(model, targetLineal.lengths);
    diffX = mean(currentLineal.x - targetLineal.x, 'omitnan');
    diffY = mean(currentLineal.y - targetLineal.y, 'omitnan');
    diffZ = mean(currentLineal.z - targetLineal.z, 'omitnan');

    adjustment = mean([diffX, diffY, diffZ], 'omitnan');
    if adjustment > 0.02
        % 线要素概率偏高，削弱长程连通
        cutMask = rand(size(model)) < 0.01;
        model(cutMask) = false;
    elseif adjustment < -0.02
        % 线要素概率偏低，增强长程连通
        connectMask = imdilate(model, strel('cube', 3));
        model = model | connectMask;
    end
end

function [gapValue, gapType] = detectDominantSpatialGap(currentSpatial, targetSpatial)
    % 识别当前模型与目标在空间特征上的主要差异
    gapMetrics = struct('minkowski', 0, 'radial', 0, 'variogram', 0, ...
        'entropy', 0, 'lineal', 0);

    if isfield(currentSpatial, 'minkowskiMeasures') && isfield(targetSpatial, 'minkowskiMeasures')
        currM = currentSpatial.minkowskiMeasures;
        tgtM = targetSpatial.minkowskiMeasures;
        diffVol = abs(currM.volumeFraction - tgtM.volumeFraction) / (tgtM.volumeFraction + 1e-3);
        diffSurf = abs(currM.surfaceArea - tgtM.surfaceArea) / (abs(tgtM.surfaceArea) + 1);
        diffBreadth = abs(currM.meanBreadth - tgtM.meanBreadth) / (abs(tgtM.meanBreadth) + 1e-3);
        diffEuler = abs(currM.eulerCharacteristic - tgtM.eulerCharacteristic) / (abs(tgtM.eulerCharacteristic) + 1);
        gapMetrics.minkowski = mean([diffVol, diffSurf, diffBreadth, diffEuler]);
    end

    if isfield(currentSpatial, 'radialDistribution') && isfield(targetSpatial, 'radialDistribution')
        currRD = currentSpatial.radialDistribution;
        tgtRD = targetSpatial.radialDistribution;
        minLen = min(length(currRD.probability), length(tgtRD.probability));
        if minLen > 0
            gapMetrics.radial = mean(abs(currRD.probability(1:minLen) - tgtRD.probability(1:minLen)));
        end
    end

    if isfield(currentSpatial, 'directionalVariogram') && isfield(targetSpatial, 'directionalVariogram')
        currVar = interpolateVariogram(currentSpatial.directionalVariogram, targetSpatial.directionalVariogram.lags);
        tgtVar = interpolateVariogram(targetSpatial.directionalVariogram, targetSpatial.directionalVariogram.lags);
        gapMetrics.variogram = mean(abs(currVar - tgtVar) ./ (abs(tgtVar) + 1e-3), 'omitnan');
    end

    if isfield(currentSpatial, 'localPorosityEntropy') && isfield(targetSpatial, 'localPorosityEntropy')
        currEntropy = currentSpatial.localPorosityEntropy.entropy;
        tgtEntropy = targetSpatial.localPorosityEntropy.entropy;
        minLen = min(length(currEntropy), length(tgtEntropy));
        if minLen > 0
            denom = max(max(tgtEntropy(1:minLen)), 1e-3);
            gapMetrics.entropy = mean(abs(currEntropy(1:minLen) - tgtEntropy(1:minLen)) / denom);
        end
    end

    if isfield(currentSpatial, 'multiScaleLinealPath') && isfield(targetSpatial, 'multiScaleLinealPath')
        currLineal = currentSpatial.multiScaleLinealPath;
        tgtLineal = targetSpatial.multiScaleLinealPath;
        diffX = mean(abs(currLineal.x - tgtLineal.x), 'omitnan');
        diffY = mean(abs(currLineal.y - tgtLineal.y), 'omitnan');
        diffZ = mean(abs(currLineal.z - tgtLineal.z), 'omitnan');
        gapMetrics.lineal = mean([diffX, diffY, diffZ], 'omitnan');
    end

    gapTypes = fieldnames(gapMetrics);
    gapValues = struct2array(gapMetrics);
    [gapValue, idx] = max(gapValues);
    gapType = gapTypes{idx};
...
function score = evaluateInitialCandidate(model, optParams)
    % 结合多种能量项评估候选模型质量
    features = extractEfficientClusterFeatures(model);
    spatial = computeEnhancedSpatialFeatures(model);
    morph = computeDetailedMorphologyFeatures(model);
    multiScale = computeMultiScaleSpatialFeatures(model);

    E_porosity = calculatePorosityEnergy(features, optParams);
    E_cluster = calculateClusterEnergy(features, optParams);
    E_morphology = calculateMorphologyEnergy(morph, optParams);
    E_spatial = calculateSpatialEnergy(spatial, optParams);
    E_connectivity = calculateConnectivityEnergy(spatial, optParams);
    E_multi = calculateMultiScaleEnergy(multiScale, optParams);

    score = E_porosity + 0.8 * E_cluster + 0.8 * E_morphology + ...
        1.2 * E_spatial + 0.6 * E_connectivity + 0.5 * E_multi;
end

function model = targetedInitialRefinement(model, optParams)
    % 在初始模型阶段执行额外的局部退火，提高与目标数据的吻合度
    ...
    for iter = 1:8
        ...
        if spatialMatch > 0.95 && morphMatch > 0.95
            break;
        end

        [gapValue, gapType] = detectDominantSpatialGap(currentSpatial, targetSpatial);
        if gapValue > 0.1
            switch gapType
                case 'radial'
                    if isfield(targetSpatial, 'radialDistribution')
                        model = enforceRadialDistribution(model, targetSpatial.radialDistribution);
                    end
                case 'minkowski'
                    if isfield(targetSpatial, 'minkowskiMeasures')
                        model = enforceMinkowskiAlignment(model, targetSpatial.minkowskiMeasures);
                    end
                case 'variogram'
                    if isfield(targetSpatial, 'directionalVariogram')
                        model = balanceDirectionalVariogram(model, targetSpatial.directionalVariogram);
                    end
                case 'entropy'
                    if isfield(targetSpatial, 'localPorosityEntropy')
                        model = adjustLocalPorosityEntropy(model, targetSpatial.localPorosityEntropy);
                    end
                case 'lineal'
                    if isfield(targetSpatial, 'multiScaleLinealPath')
                        model = balanceLinealPath(model, targetSpatial.multiScaleLinealPath);
                    end
            end
        end

        if spatialMatch < morphMatch
            model = applySpatialCorrection(model, currentSpatial, targetSpatial);
        else
            model = applyMorphologyCorrection(model, currentMorph, targetMorph);
        end
...
function E_spatial = calculateSpatialEnergy(spatialFeatures, optParams)
    ...
    % 定向变差函数
    if isfield(spatialFeatures, 'directionalVariogram') && ...
            isfield(targetSpatial, 'directionalVariogram')
        currVar = spatialFeatures.directionalVariogram;
        tgtVar = targetSpatial.directionalVariogram;
        commonLags = intersect(currVar.lags, tgtVar.lags);
        if ~isempty(commonLags)
            currInterp = interpolateVariogram(currVar, commonLags);
            tgtInterp = interpolateVariogram(tgtVar, commonLags);
            diffVal = mean(abs(currInterp - tgtInterp) ./ (abs(tgtInterp) + 1e-3), 'omitnan');
            E_spatial = E_spatial + diffVal;
            nTerms = nTerms + 1;
        end
    end
    ...
    % 径向分布
    if isfield(spatialFeatures, 'radialDistribution') && ...
            isfield(targetSpatial, 'radialDistribution')
        currRD = spatialFeatures.radialDistribution;
        tgtRD = targetSpatial.radialDistribution;
        minLen = min(length(currRD.probability), length(tgtRD.probability));
        if minLen > 0
            diffVal = mean(abs(currRD.probability(1:minLen) - tgtRD.probability(1:minLen)));
            E_spatial = E_spatial + diffVal;
            nTerms = nTerms + 1;
        end
    end
    ...
    % 局部孔隙率熵
    if isfield(spatialFeatures, 'localPorosityEntropy') && ...
            isfield(targetSpatial, 'localPorosityEntropy')
        currEntropy = spatialFeatures.localPorosityEntropy.entropy;
        tgtEntropy = targetSpatial.localPorosityEntropy.entropy;
        minLen = min(length(currEntropy), length(tgtEntropy));
        if minLen > 0
            denom = max(max(tgtEntropy(1:minLen)), 1e-3);
            diffVal = mean(abs(currEntropy(1:minLen) - tgtEntropy(1:minLen)) / denom);
            E_spatial = E_spatial + diffVal;
            nTerms = nTerms + 1;
        end
    end
    ...
    % Minkowski泛函
    if isfield(spatialFeatures, 'minkowskiMeasures') && ...
            isfield(targetSpatial, 'minkowskiMeasures')
        currM = spatialFeatures.minkowskiMeasures;
        tgtM = targetSpatial.minkowskiMeasures;
        diffVol = abs(currM.volumeFraction - tgtM.volumeFraction) / (tgtM.volumeFraction + 1e-3);
        diffSurf = abs(currM.surfaceArea - tgtM.surfaceArea) / (abs(tgtM.surfaceArea) + 1);
        diffBreadth = abs(currM.meanBreadth - tgtM.meanBreadth) / (abs(tgtM.meanBreadth) + 1e-3);
        diffEuler = abs(currM.eulerCharacteristic - tgtM.eulerCharacteristic) / (abs(tgtM.eulerCharacteristic) + 1);
        diffVal = mean([diffVol, diffSurf, diffBreadth, diffEuler]);
        E_spatial = E_spatial + diffVal;
        nTerms = nTerms + 1;
    end
    ...
    % 多尺度线要素
    if isfield(spatialFeatures, 'multiScaleLinealPath') && ...
            isfield(targetSpatial, 'multiScaleLinealPath')
        currLineal = spatialFeatures.multiScaleLinealPath;
        tgtLineal = targetSpatial.multiScaleLinealPath;
        diffX = mean(abs(currLineal.x - tgtLineal.x), 'omitnan');
        diffY = mean(abs(currLineal.y - tgtLineal.y), 'omitnan');
        diffZ = mean(abs(currLineal.z - tgtLineal.z), 'omitnan');
        diffVal = mean([diffX, diffY, diffZ], 'omitnan');
        E_spatial = E_spatial + diffVal;
        nTerms = nTerms + 1;
    end
...
function newModel = applyMoveToModel(model, move, validIndices)
    % 将移动应用到模型的副本上
    newModel = model;
    linearIdx = move.linearIdx(validIndices);
    newValues = move.newValues(validIndices);

    newModel(linearIdx) = newValues;

    if islogical(model)
        newModel = logical(newModel);
    else
        newModel = reshape(newModel, size(model));
    end
end

function [mcmcState, accepted] = applyBestComprehensiveMoves(mcmcState, moves, ...
    deltaEnergies, temperature, moveTypes)
    % 应用最佳综合移动，并在接受时进行完整的能量校验
    accepted = false;
    ...
    % 构造候选状态并精确计算能量变化
    proposedState = mcmcState;
    proposedState.model = applyMoveToModel(mcmcState.model, bestMove, validIndices);
    proposedState = updateAllFeatures(proposedState);

    exactDelta = proposedState.currentEnergy - mcmcState.currentEnergy;
    acceptProb = min(1, exp(-exactDelta / max(temperature, 1e-6)));

    if exactDelta < 0 || rand() < acceptProb
        % 接受移动，更新最佳状态
        if proposedState.currentEnergy < mcmcState.bestEnergy
            proposedState.bestEnergy = proposedState.currentEnergy;
            proposedState.bestModel = proposedState.model;
        else
            proposedState.bestEnergy = mcmcState.bestEnergy;
            proposedState.bestModel = mcmcState.bestModel;
        end

        % 记录接受的移动类型
        if ~isfield(proposedState, 'acceptedMoveTypes')
            proposedState.acceptedMoveTypes = struct();
        end
        if isfield(proposedState.acceptedMoveTypes, bestType)
            proposedState.acceptedMoveTypes.(bestType) = ...
                proposedState.acceptedMoveTypes.(bestType) + 1;
        else
            proposedState.acceptedMoveTypes.(bestType) = 1;
        end
...
function match = calculateSpatialMatch(currentFeatures, targetFeatures)
    ...
    % 定向变差函数匹配
    if isfield(currentFeatures, 'directionalVariogram') && ...
            isfield(targetFeatures, 'directionalVariogram')
        currVar = currentFeatures.directionalVariogram;
        tgtVar = targetFeatures.directionalVariogram;
        commonLags = intersect(currVar.lags, tgtVar.lags);
        if ~isempty(commonLags)
            currInterp = interpolateVariogram(currVar, commonLags);
            tgtInterp = interpolateVariogram(tgtVar, commonLags);
    ...
    % 径向分布匹配
    if isfield(currentFeatures, 'radialDistribution') && ...
            isfield(targetFeatures, 'radialDistribution')
        currRD = currentFeatures.radialDistribution;
        tgtRD = targetFeatures.radialDistribution;
        minLen = min(length(currRD.probability), length(tgtRD.probability));
        if minLen > 0
            diffVal = mean(abs(currRD.probability(1:minLen) - tgtRD.probability(1:minLen)));
            match = match + (1 - min(diffVal * 5, 1));
            nTerms = nTerms + 1;
        end
    end

    % 局部孔隙率熵匹配
    if isfield(currentFeatures, 'localPorosityEntropy') && ...
            isfield(targetFeatures, 'localPorosityEntropy')
        currEntropy = currentFeatures.localPorosityEntropy.entropy;
        tgtEntropy = targetFeatures.localPorosityEntropy.entropy;
        minLen = min(length(currEntropy), length(tgtEntropy));
        if minLen > 0
            denom = max(max(tgtEntropy(1:minLen)), 1e-3);
            diffVal = mean(abs(currEntropy(1:minLen) - tgtEntropy(1:minLen)) / denom);
    ...
    % Minkowski泛函匹配
    if isfield(currentFeatures, 'minkowskiMeasures') && ...
            isfield(targetFeatures, 'minkowskiMeasures')
        currM = currentFeatures.minkowskiMeasures;
        tgtM = targetFeatures.minkowskiMeasures;
        diffVol = abs(currM.volumeFraction - tgtM.volumeFraction) / (tgtM.volumeFraction + 1e-3);
        diffSurf = abs(currM.surfaceArea - tgtM.surfaceArea) / (abs(tgtM.surfaceArea) + 1);
        diffBreadth = abs(currM.meanBreadth - tgtM.meanBreadth) / (abs(tgtM.meanBreadth) + 1e-3);
        diffEuler = abs(currM.eulerCharacteristic - tgtM.eulerCharacteristic) / (abs(tgtM.eulerCharacteristic) + 1);
    ...
    % 多尺度线要素匹配
    if isfield(currentFeatures, 'multiScaleLinealPath') && ...
            isfield(targetFeatures, 'multiScaleLinealPath')
        currLineal = currentFeatures.multiScaleLinealPath;
        tgtLineal = targetFeatures.multiScaleLinealPath;
        diffX = mean(abs(currLineal.x - tgtLineal.x), 'omitnan');
        diffY = mean(abs(currLineal.y - tgtLineal.y), 'omitnan');
        diffZ = mean(abs(currLineal.z - tgtLineal.z), 'omitnan');
        diffVal = mean([diffX, diffY, diffZ], 'omitnan');
        match = match + (1 - min(diffVal * 3, 1));
        nTerms = nTerms + 1;
    end
