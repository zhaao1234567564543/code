...
fprintf('正在缓存功率谱信息...\n');
powerCache = computeModelPowerSpectrum(rawBinaryModel);
spatialFeatures.powerSpectrum = powerCache.powerSpectrum;
spatialFeatures.radialPowerSpectrum = powerCache.radialPowerSpectrum;
spatialFeatures.radialFrequencies = powerCache.radialFrequencies;
...
optParams.powerSpectrum = powerCache.powerSpectrum;
optParams.radialPowerSpectrum = powerCache.radialPowerSpectrum;
optParams.radialFrequencies = powerCache.radialFrequencies;
optParams.applyInitialMorphology = ~preserve_small_pores;
...
function spectrumCache = computeModelPowerSpectrum(binaryModel)
    % 计算并缓存功率谱及其径向平均
    data = double(binaryModel);
    centered = data - mean(data(:));
    fftVol = fftn(centered);
    powerSpectrum = abs(fftVol).^2 / numel(binaryModel);
    shiftedPSD = fftshift(powerSpectrum);
    [radialProfile, radialFreq] = computeRadialAverage3D(shiftedPSD);

    spectrumCache = struct();
    spectrumCache.powerSpectrum = powerSpectrum;
    spectrumCache.radialPowerSpectrum = radialProfile;
    spectrumCache.radialFrequencies = radialFreq;
end

function [radialProfile, radii] = computeRadialAverage3D(volume)
    % 计算三维功率谱的径向平均
    dims = size(volume);
    ...
    binIdx = floor(distances(:)) + 1;
    nBins = max(binIdx);

    sums = accumarray(binIdx, volume(:), [nBins, 1], @sum, 0);
    counts = accumarray(binIdx, 1, [nBins, 1], @sum, 0);
    counts(counts == 0) = 1;
...
function model = generateInitialModel_Fourier(dims, targetPorosity, spatialFeatures, optParams)
    % 使用功率谱合成的方式生成初始模型
    cachedPSD = [];
    if isfield(optParams, 'powerSpectrum') && ~isempty(optParams.powerSpectrum)
        cachedPSD = optParams.powerSpectrum;
    elseif isfield(spatialFeatures, 'powerSpectrum') && ~isempty(spatialFeatures.powerSpectrum)
        cachedPSD = spatialFeatures.powerSpectrum;
    end

    radialPSD = [];
    radialFreq = [];
    if isfield(optParams, 'radialPowerSpectrum') && ~isempty(optParams.radialPowerSpectrum)
        radialPSD = optParams.radialPowerSpectrum;
        if isfield(optParams, 'radialFrequencies')
            radialFreq = optParams.radialFrequencies;
        end
    elseif isfield(spatialFeatures, 'radialPowerSpectrum') && ~isempty(spatialFeatures.radialPowerSpectrum)
        radialPSD = spatialFeatures.radialPowerSpectrum;
        if isfield(spatialFeatures, 'radialFrequencies')
            radialFreq = spatialFeatures.radialFrequencies;
        end
    end

    amplitude = prepareTargetAmplitude(cachedPSD, radialPSD, radialFreq, dims);
    field = synthesizeFieldFromAmplitude(amplitude);
    field = (field - mean(field(:))) / (std(field(:)) + eps);

    threshold = quantile(field(:), 1 - targetPorosity);
    model = field > threshold;
    model = adjustPorosity(model, targetPorosity);

    if isfield(optParams, 'applyInitialMorphology') && optParams.applyInitialMorphology
        se = strel('sphere', 1);
        model = imopen(imclose(model, se), se);
    end
end

function amplitude = prepareTargetAmplitude(cachedPSD, radialPSD, radialFreq, dims)
    amplitude = [];

    if ~isempty(cachedPSD)
        if isequal(size(cachedPSD), dims)
            amplitude = sqrt(max(real(cachedPSD), 0));
        elseif ~isempty(radialPSD)
            amplitude = reconstructAmplitudeFromRadial(radialPSD, radialFreq, dims);
        end
    end

    if isempty(amplitude) && ~isempty(radialPSD)
        amplitude = reconstructAmplitudeFromRadial(radialPSD, radialFreq, dims);
    end
    ...
    amplitude(1,1,1) = 0; % 去除直流分量
end

function amplitude = reconstructAmplitudeFromRadial(radialPSD, radialFreq, dims)
    if isempty(radialPSD)
        amplitude = [];
        return;
    end

    radialPSD = double(radialPSD(:));
    if nargin < 2 || isempty(radialFreq)
        radialFreq = (0:length(radialPSD)-1)';
    else
        radialFreq = double(radialFreq(:));
    end

    % 构建频率网格
    freqX = (-floor(dims(1)/2)):(ceil(dims(1)/2)-1);
    freqY = (-floor(dims(2)/2)):(ceil(dims(2)/2)-1);
    freqZ = (-floor(dims(3)/2)):(ceil(dims(3)/2)-1);
    [FX, FY, FZ] = ndgrid(freqX, freqY, freqZ);
    radii = sqrt(FX.^2 + FY.^2 + FZ.^2);

    maxFreq = max(radialFreq);
    interpVals = interp1(radialFreq, radialPSD, min(radii, maxFreq), 'linear', 'extrap');
    interpVals(radii > maxFreq) = radialPSD(end);

    amplitudeShifted = sqrt(max(interpVals, 0));
    amplitude = ifftshift(amplitudeShifted);
end

function field = synthesizeFieldFromAmplitude(amplitude)
    dims = size(amplitude);
    whiteNoise = randn(dims);
    noiseFFT = fftn(whiteNoise);
    phase = noiseFFT ./ (abs(noiseFFT) + eps);
    spectralField = amplitude .* phase;
    field = real(ifftn(spectralField));
end
...
function mcmcModel = generateComprehensiveInitialModel(dims, targetPorosity, ...
    ...
    model = generateInitialModel_Fourier(dims, targetPorosity, spatialFeatures, optParams);

    if ~isempty(morphologyFeatures.sphericity)
        targetSphericity = mean(morphologyFeatures.sphericity);
        if targetSphericity > 0.6
            smoothed = imgaussfilt3(double(model), 2);
            model = smoothed > 0.5;
        else
            filtered = anisotropicFilter3D(double(model), targetAnisotropy);
            model = filtered > 0.5;
        end
        model = adjustPorosity(model, targetPorosity);
    end
